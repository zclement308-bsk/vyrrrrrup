...
export function FlowTop({ step, onBack, total, displayStep }: { step: number; onBack: () => void; total?: number; displayStep?: number }) {
  const denom = total ?? questions.length
  const shown = displayStep ?? step
  const progress = Math.round(((shown + 1) / denom) * 100)
  return (
    <header className="flow-top">
      ...
      <span className="mono-note">{String(shown + 1).padStart(2, '0')} / {String(denom).padStart(2, '0')}</span>
    </header>
  )
}
